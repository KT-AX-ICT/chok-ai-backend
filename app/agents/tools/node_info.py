"""Tool ④ 노드 정보 조회.

Tool①~③ 분석 결과에 노드(하드웨어 위치) 컨텍스트를 보강한다. ``node_id`` 문자열을
받아 하드웨어 계층과 과거 이상 발생 비율을 반환한다. (비정상 경로에서만 호출)

- node_id 파싱: rack/midplane/node_slot/node_role/socket_position/processor_unit 분리.
  노드카드는 16진수 → 10진수로 변환한다(예: ``NB`` → ``N11``).
- alert_pct 조회: ``metadata/node_stats.json`` 에서 해당 노드의 사전 집계 이상 비율(%).
  랙/미드플레인 단위 조회 시 하위 노드 중 최대 alert_pct 를 반환한다.
- 파싱 불가(NULL·UNKNOWN_LOCATION·형식 불일치) → node_metadata 전 필드 None.
- 미등록 노드 → alert_stats=None.

LangGraph 등 오케스트레이션 프레임워크에 의존하지 않는 순수 함수로 작성한다.
"""

import json
import re
from functools import lru_cache
from pathlib import Path

from pydantic import BaseModel, Field

NODE_STATS_PATH = Path(__file__).parent / "metadata" / "node_stats.json"

_NC_HEX  = {f"{i:X}": str(i) for i in range(16)}  # 노드카드 16진수(0~F) → 10진수 변환 테이블
_NODE_RE = re.compile(                              # BGL 노드 ID 계층 파싱 정규식 (R-M-N-C/I:J-U)
    r"R(?P<rack>[0-9A-Fa-f]+)"
    r"(?:-M(?P<midplane>\d)"
    r"(?:-N(?P<nodecard>[0-9A-Fa-f]+)"
    r"(?:-(?P<ntype>C|I)"
    r"(?::J(?P<jslot>\d+)-U(?P<unit>\d+))?)?)?)?"
    r"$"
)


# ── Pydantic 출력 모델 ────────────────────────────────────────────────────────

class NodeMetadata(BaseModel):
    """node_id 파싱 결과 (하드웨어 계층). 파싱 불가 시 전 필드 None."""

    rack: str | None = Field(
        default=None, description="랙 식별자. 예: 'R30'."
    )
    midplane: str | None = Field(
        default=None, description="미드플레인. 예: 'R30-M0'."
    )
    node_slot: str | None = Field(
        default=None, description="노드 슬롯(노드카드 16진수→10진수 변환). 예: 'N9'."
    )
    node_role: str | None = Field(
        default=None, description="노드 역할. 'Compute' 또는 'I/O'."
    )
    socket_position: str | None = Field(
        default=None, description="소켓 위치. 예: 'J16'."
    )
    processor_unit: str | None = Field(
        default=None, description="프로세서 유닛. 예: 'U01'."
    )


class AlertStats(BaseModel):
    """노드의 사전 집계 이상 통계."""

    alert_pct: float = Field(
        description="2k 샘플 기준 전체 이상 로그 중 이 노드 발생 비율(%)."
    )


class NodeInfoResult(BaseModel):
    """Tool④ 출력 모델 (내부 Tool 결과 모델)."""

    node_metadata: NodeMetadata = Field(
        description="node_id 파싱 결과(하드웨어 계층)."
    )
    alert_stats: AlertStats | None = Field(
        description="사전 집계 이상 비율. 미등록 노드는 None."
    )


# ── 노드 ID 파서 ─────────────────────────────────────────────────────────────

@lru_cache(maxsize=None)
def parse_node_id(node_id: str) -> NodeMetadata:
    if not node_id or node_id in ("NULL", "UNKNOWN_LOCATION"):
        return NodeMetadata()

    m = _NODE_RE.match(node_id)
    if not m or not m.group("rack"):
        return NodeMetadata()

    rack  = m.group("rack")
    mp    = m.group("midplane")
    nc    = m.group("nodecard")
    ntype = m.group("ntype")
    jslot = m.group("jslot")
    unit  = m.group("unit")

    nc_num = _NC_HEX.get(nc, nc) if nc else None

    return NodeMetadata(
        rack=f"R{rack}",
        midplane=f"R{rack}-M{mp}"                        if mp    is not None else None,
        node_slot=f"N{nc_num}"                           if nc    is not None else None,
        node_role=("Compute" if ntype == "C" else "I/O") if ntype is not None else None,
        socket_position=f"J{jslot}"                      if jslot is not None else None,
        processor_unit=f"U{unit}"                        if unit  is not None else None,
    )


# ── 데이터 로더 ───────────────────────────────────────────────────────────────

@lru_cache
def _load_alert_stats(path: str = str(NODE_STATS_PATH)) -> dict:
    if not Path(path).exists():
        return {}
    with open(path, encoding="utf-8") as f:
        return json.load(f).get("nodes", {})


# ── alert_pct 조회 ────────────────────────────────────────────────────────────

def _get_alert_stats(node_id: str, stats: dict) -> AlertStats | None:
    stat = stats.get(node_id)
    if stat is None and node_id.startswith("R"):
        # 랙/미드플레인 단위 조회 시 하위 노드 중 alert_pct 가 가장 높은 항목 반환
        candidates = [
            v for k, v in stats.items()
            if k == node_id or k.startswith(node_id + "-") or k.startswith(node_id + ":")
        ]
        if candidates:
            stat = max(candidates, key=lambda x: x.get("alert_pct", 0))
    if stat is None:
        return None
    return AlertStats(alert_pct=stat.get("alert_pct", 0.0))


# ── 진입점 ───────────────────────────────────────────────────────────────────

def get_node_info(node_id: str) -> NodeInfoResult:
    node_id = (node_id or "").strip().upper()
    return NodeInfoResult(
        node_metadata=parse_node_id(node_id),
        alert_stats=_get_alert_stats(node_id, _load_alert_stats()),
    )
