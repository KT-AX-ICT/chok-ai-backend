...
| 패턴 seed 처리 | 고유경, 윤혜림 | 이예지 | Tool③ 매칭용 패턴 seed 파일(`app/data/clusters.json`)을 적재·로드한다. `app/data/patterns.py`에서 event_id → cluster_id로 매칭. |

## Scope

- BGL 로그 구조 확인용 유틸
- FastAPI 분석 입력 구조 검토
- Tool③ 매칭용 패턴 seed 파일(`clusters.json`) 적재·로드 (`app/data/patterns.py`)
