...
- **DTO 정의 (`models/` 폴더 하위):**
  ...
  > [참고] API 명세([API.md](API.md)) 기반 작성. 요청에서 `label`·`eventId` 제외, 응답에 `eventId` 포함
...
- **④ Node별 정보 조회 Tool:** `node` 기준 부가 정보를 조회해 분석 컨텍스트로 제공 (①에 의존하지 않고 독립 실행 가능)

## 4단계: LangGraph 기반 AI Agent 구축
개발한 4개 도구를 LangGraph `StateGraph`의 노드로 명시적으로 연결하여, 분류·조회 Tool이 **항상 실행되도록** 분석 흐름을 구성합니다. (Tool 호출 여부를 LLM의 임의 판단에 맡기지 않고, 그래프로 순서를 고정)
> 상세 구현 목록: [step4_agent.md](step4_agent.md)
- **State 정의:** 원본 로그, `eventId`, 이상 여부·`riskLevel`, `clusterId`, Node 컨텍스트, 최종 분석 결과를 담는 상태(`TypedDict`) 설계
- **노드 구성 (`services/agent_service.py`):**
  - `이벤트 템플릿 분류 노드`: ① Tool 호출 → State에 `eventId` 기록 (**선행 노드**)
  - `이상 여부 + 긴급도 분류 노드`: ② Tool 호출 → State에 이상 여부·`riskLevel` 기록 (템플릿 결과 사용)
  - `클러스터 분류 노드`: ③ Tool 호출 → State에 `clusterId` 기록 (템플릿 결과 사용)
  - `Node 정보 조회 노드`: ④ Tool 호출 → State에 Node 컨텍스트 기록 (독립 실행)
  - `LLM 분석 노드`: 확보된 컨텍스트와 원본 로그를 LLM에 전달하여 분석 내용·대응 방안 작성
  - `결과 매핑 노드`: LLM 응답을 JSON으로 파싱하여 API 응답 규격에 매핑 (`eventId` 포함)
- **엣지 연결:** `START → 이벤트 템플릿 분류 → (이상 여부+긴급도 · 클러스터 분류) → LLM 분석 → 결과 매핑 → END`. 단, `Node 정보 조회`는 `START`에서 병렬 분기해 `LLM 분석`에서 합류 (①에 의존하지 않음)
