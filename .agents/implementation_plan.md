...
- **DTO 정의 (`models/` 폴더 하위):**
  ...
  - **공통 타입:** `RiskLevel`(Literal 한글), `ProcessStatus`(Enum `success`/`fail`). `isAbnormal`은 `bool`(이상=true/정상=false, Spring DB 정의)
  - **분석 결과 본문(공통):** `AnalysisResult` (단건·배치 공유, 개별 로그 `analyzedAt` 포함)
  - **단건 분석 모델:** Request(`LogAnalyzeRequest`), Response(`LogAnalyzeResponse` — `isAbnormal` 포함, `result`는 정상·이상 모두 포함)
  - **다건 분석 모델:** Request(`LogBatchAnalyzeRequest`), Response(`LogBatchAnalyzeResponse`, `LogBatchResultItem` — `processStatus`/`isAbnormal` 구분)
  - **에러 모델:** `ErrorResponse` (`code`·`message`·`detail`)
  > [참고] API 명세([API.md](API.md)) 기반 작성. 요청에서 `label`·`eventId` 제외, 응답 `result`에 `eventId`(이상 로그만, 정상이면 null) + 응답에 `isAbnormal`(이상 여부, bool: 이상=true/정상=false) 포함
...
- **State 정의:** 원본 로그, `eventId`, 이상 여부(`isAbnormal`)·`riskLevel`, `clusterId`, Node 컨텍스트, 최종 분석 결과를 담는 상태(`TypedDict`) 설계
- **노드 구성 (`services/agent_service.py`):**
  - `이벤트 템플릿 분류 노드`: ① Tool 호출 → State에 `eventId` 기록 (**선행 노드**)
  - `이상 여부 + 긴급도 분류 노드`: ② Tool 호출 → State에 이상 여부(`isAbnormal`)·`riskLevel` 기록 (**분기 기준 노드**)
  ...
  - `결과 매핑 노드`: LLM 구조화 출력(`with_structured_output`)을 받아 API 응답 규격에 매핑 (`isAbnormal` 포함, `result`에 `eventId`; `정상`이면 `result`에 정상 사유만)
- **엣지 연결:**
