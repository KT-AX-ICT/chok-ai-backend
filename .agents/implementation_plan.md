...
- **DTO 정의 (`models/` 폴더 하위):**
  ...
  - **공통 타입:** `RiskLevel`(Literal 한글), `ProcessStatus`(Enum `success`/`fail`). `status`는 `bool`(이상=true/정상=false, Spring DB 정의)
  ...
  > [참고] API 명세([API.md](API.md)) 기반 작성. 요청에서 `label`·`eventId` 제외, 응답 `result`에 `eventId`(이상 로그만, 정상이면 null) + 응답에 `status`(이상 여부, bool: 이상=true/정상=false) 포함
...
- **노드 구성 (`services/agent_service.py`):**
  ...
  - `결과 매핑 노드`: LLM 구조화 출력(`with_structured_output`)을 받아 API 응답 규격에 매핑 (`status` 포함, `result`에 `eventId`; `정상`이면 `result`에 정상 사유만)
- **엣지 연결:**
